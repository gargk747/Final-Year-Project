...
class View(object):
    ...
    def getCorrelation(self,series_id):
        if self.fx.type==Type.temporal:
            data1=[i for i in range(int(self.tuple_num/self.series_num))]
        else:
    ...
    def getM(self):
        if self.chart==Chart.pie:
            if self.tuple_num==1:
                ...
                sumY=sum(self.Y[0])
                self.M=reduce(lambda x,y:x+y,[-(1.0*y/sumY)*math.log(1.0*y/sumY) for y in self.Y[0]])
            elif self.tuple_num>10:
                sumY=sum(self.Y[0])
                self.M=reduce(lambda x,y:x+y,[-(1.0*y/sumY)*math.log(1.0*y/sumY) for y in self.Y[0]])*10.0/self.tuple_num
    ...
    def output(self,order):
        ...
        if self.fx.type == Type.numerical:
            ...
            len_x = len(self.X)
            x_data = '[' + reduce(lambda s1, s2: s1 + s2, [str(list(map(str, self.X[i]))) for i in range(len_x)]).replace("'",'"') + ']'
        y_data = str(self.Y)
